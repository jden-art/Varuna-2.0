...
#define FIREBASE_AUTH "AIzaSyAAvL-yCNXxDglQYc5aIMVhUZCJ7XXIOVo"
// Alternative: use Firebase Auth ID token flow
...
// ============================================================================
//
// Updated for ESP32 Arduino Core v3.x timer API
//
...
// Forward declaration of timer ISR (must be declared before initSoftwareSerialRx)
...
void initSoftwareSerialRx() {
    ...
    // Initialize hardware timer at 1MHz (1µs per tick)
    // v3.x API: timerBegin(frequency) — replaces timerBegin(num, prescaler, countUp)
    swSerialTimer = timerBegin(1000000);

    // v3.x API: timerAttachInterrupt(timer, fn) — no edge parameter
    timerAttachInterrupt(swSerialTimer, swSerialBitTimerISR);

    // Don't start yet — timer starts on each start bit detection
    timerStop(swSerialTimer);
...
void IRAM_ATTR swSerialStartBitISR() {
    ...
    // Reset counter and set alarm for 1.5 bit times to sample middle of first data bit
    // v3.x API: timerRestart() + timerAlarm(timer, value, autoreload, reload_count)
    // autoreload=false, reload_count=0 → one-shot
    timerRestart(swSerialTimer);
    timerAlarm(swSerialTimer, SW_SERIAL_BIT_US + (SW_SERIAL_BIT_US / 2), false, 0);
}
...
void IRAM_ATTR swSerialBitTimerISR() {
    ...
    if (swBitCount < 8) {
        ...
        // Set timer for next interval (1 bit time)
        // This handles both "next data bit" and "stop bit wait" cases
        timerRestart(swSerialTimer);
        timerAlarm(swSerialTimer, SW_SERIAL_BIT_US, false, 0);
...
bool downloadOTABinaryToSD(const char* sdPath) {
    ...
    // GitHub releases redirect through multiple CDN domains.
    // HTTPC_FORCE_FOLLOW_REDIRECTS handles cross-domain redirects.
    // Retry loop handles intermittent TLS failures with GitHub's CDN.
    ...
    for (int attempt = 1; attempt <= 3; attempt++) {
        WiFiClientSecure secureClient;
        secureClient.setInsecure();
        secureClient.setTimeout(30);  // 30 second TLS handshake timeout

        http.begin(secureClient, String(otaUrl));
        http.setFollowRedirects(HTTPC_FORCE_FOLLOW_REDIRECTS);  // cross-domain redirects
        http.setTimeout(60000);
        http.setUserAgent("ESP32-C3");  // some CDNs reject empty user-agent
        ...
        snprintf(retryMsg, sizeof(retryMsg),
                 "OTA download attempt %d/3 failed: HTTP %d — retrying...",
                 attempt, httpCode);
        logMsg("WARN", retryMsg);
    ...
    if (!downloadStarted) {
        char msg[128];
        snprintf(msg, sizeof(msg), "OTA download failed after 3 attempts: HTTP %d", httpCode);
        logMsg("ERROR", msg);
        http.end();
        return false;
    }
