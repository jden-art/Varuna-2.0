...
> **See Section 15** for the full BMP280 dual-role explanation — atmospheric baseline tracking and depth measurement — including the pressure formula, mode transition thresholds, and baseline maintenance rules. The formula `H = L + ΔP/(ρg)` used here is derived and calibrated there.
...
**Buffer flush on interval change:** When sampling interval changes dramatically (e.g., from 30min to 2min during escalation), old readings at the long interval are invalidated — the buffer is filled with the last reading to avoid comparing readings taken 30 minutes apart with ones taken 2 minutes apart. The specific function responsible (`flushSustainedBufferIfIntervalChanged()`) and its triggering condition are detailed in Section 21.

> **See Section 21** for the complete implementation of this algorithm, including the circular buffer mechanics, the full step-by-step logic, and the buffer flush function. The above is a design summary; Section 21 is the authoritative implementation reference.
...
│              OR server-triggered via GSM command)            │
│  ├── STATUS → device replies with current state              │
│  ├── Maintenance commands as documented                      │
│  └── DIAG_* → diagnostic suite commands                      │
│     NOTE: The server can also initiate diagnostic queries    │
│     by sending SMS to the device (e.g. DIAG_TETHER,         │
│     DIAG_ANCHOR). This means the communication path is       │
│     bidirectional: device → server (GPRS) AND               │
│     server → device (SMS diagnostic commands).               │
│     See Section 17.5 for the full server-triggered           │
│     diagnostic command set and Section 29.6 for the          │
│     SMS command authorization model.                         │
│                                                              │
│  NOT USED FOR:                                               │
...
54-133 80   Authorized field engineer numbers
             (5 × phone number × 16 bytes = 80 bytes, for diagnostic SMS)
             NOTE: original table showed 54-69 (16 bytes) which is wrong;
             5 numbers × 16 chars = 80 bytes, correct range is 54–133.
134   1     Authorized count
135   1     Authorized magic (0xC3)

136   4     Origin latitude (float)
140   4     Origin longitude (float)
144   4     Geofence radius (float, meters)
148   1     Origin magic (0xGF)

149-289     RESERVED (141 bytes — reserved for future use or padding)
...
> **This section expands on the MODE 3 operating context introduced in Section 2.** The depth formula `H = L + ΔP/(ρg)` appears in Section 2's Mode 3 block as the measurement equation; this section defines how the BMP280 establishes the atmospheric baseline required for that formula, the mode transition thresholds that trigger it, and the cross-validation logic between tilt and pressure readings.
...
STAGED RESPONSE:
  ...
  3.40-3.50V  10-20%     LOW           • Extend sample intervals
                                       • Reduce GPRS to conserve mode
                                       • Reduce obstruction light
                                         duty cycle
                                       • Report LOW in HTTP POST
                                       (See Section 6: BATTERY_LOW_THRESH = 20%,
                                        piecewise curve places ~3.50V at 20%
                                        and ~3.40V at 10%)

  3.30-3.40V  5-10%      CRITICAL      • Emergency mode:
                                         measure + transmit 1x/hour
                                       • Disable obstruction lights (see Section 11
                                         for the obstruction light subsystem;
                                         see Section 6 for how GPRS:CONSERVE
                                         interacts with this battery state)
                                       • Force EEPROM save
                                       • Report CRITICAL in HTTP POST
                                       • GPRS:CONSERVE active (see Section 6)
                                       (See Section 6: BATTERY_CRITICAL_THRESH = 10%,
                                        piecewise curve places ~3.30V at 5%)

  3.00-3.30V  0-5%       STEEP DROP   • Same as CRITICAL actions above;
                                         steep final discharge, cutoff imminent
...
> **This section expands on the sustained rise design introduced in Section 4.** Section 4 presents the conceptual summary and worked examples; this section is the implementation reference, including the `flushSustainedBufferIfIntervalChanged()` function.
...
Override 1: Cold Start
  ...
  → See Section 23 for the complete cold-start logic,
    including how the first reading directly sets response level.

Override 2: Extreme Rate
  IF ratePer15Min > 30 cm/15min AND zone ≥ ALERT
  → Force sustained = true
  → Reason: Flash flood; 30cm/15min is ~2cm/minute
  → See Section 20 for how ratePer15Min is computed and
    the 30 cm/15min threshold in context of all rate categories.

Override 3: Catastrophic Rate
  IF ratePer15Min > 50 cm/15min (any zone)
  → Force sustained = true
  → Reason: Dam break / tsunami; override everything
  → See Section 20 for the 50 cm/15min threshold and the
    rate clamping logic (200 cm/15min max to reject glitches).
```
...
> **This section is the full expansion of Override 1 from Section 22.** Section 22 defines the condition (`readingsSinceBoot ≤ 4 AND zone ≥ WARNING → force sustained = true`); this section describes what happens on the actual first reading before that override even engages — the immediate response level assignment that bypasses the normal matrix evaluation entirely.
...
│                                                                    │
│  NOTE: processCommand() handles 35 commands (33 unique actions) in 5 categories:     │
...
│  │     (35 commands: diagnostics, maintenance, config, lights) │   │
...
>
> Total: 35 commands (33 unique actions). GETOBLIGHT and GETOBPRESETS are functionally diagnostic but counted in the obstruction light group; see Section 29.8. No ambiguity about when or why each one is used.
...
│                                  │                                          │                                             │
│ 35 commands (33 unique           │ Every command answers "when would someone│ Reduces flexibility for edge cases; engineer  │
│ actions), strictly categorized   │ type this on a deployed device?" — if    │ who needs remote recalibration must retrieve │
│ (see Section 29.8)               │ "never", command removed from console    │ the device                                   │
│                                  │                                          │                                             │
